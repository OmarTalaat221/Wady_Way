// app/community/community-details/page.jsx
"use client";
...
const CommunityDetailsPage = () => {
  ...
  const contentId = searchParams.get("community_id");
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      setCurrentUrl(
        `${window.location.origin}${pathname}?community_id=${contentId}`
      );
    }
  }, [pathname, contentId]);
  ...
  const shareLinks = {
    ...
    x: () => {
      if (typeof window === "undefined" || !currentUrl) return;
      const text = encodeURIComponent(contentData?.title || "");
  ...
  const handleReplySubmit = async (commentId) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${base_url}/user/community/make_a_comment.php`,
        replyData,
        {
          headers: {
            "Content-Type": "application/json",
  ...
  if (!contentData) {
    return (
      <div className="text-center py-20">
        <h4 className="text-gray-500">Content not found</h4>
      </div>
    );
  }
  ...
  return (
    <>
      <Breadcrumb pagename="Community Details" pagetitle="Community Details" />
      <div className="blog-details-section pt-[50px] mb-[80px]">
        <div className="container">
          <div className="row g-lg-4 gy-5 justify-content-center">
            <div className="col-lg-8">
              <div className="post-thumb mb-30">
                <img
                  src={contentData.cover_image}
              ...
              <div className="blog-meta two mb-50">
                <div className="author-area">
                  <div className="author-img">
                    <img
                      src={contentData.user_data.image}
                      alt={contentData.user_data.full_name}
                  ...
                  <div className="author-content">
                    <h6>
                      By,{" "}
                      <Link href="/community">
                        {contentData.user_data.full_name}
                      </Link>
                    </h6>
                  </div>
                </div>
                <ul>
                  ...
                  <li>
                    ...
                    {calculateReadTime(contentData.description)} Min reads
                  </li>
                  <li>
                    <a href="#comment-area">
                      <svg
                      ...
                      {contentData.comments.length} Comment
                      {contentData.comments.length !== 1 ? "s" : ""}
                    </a>
                  </li>
                </ul>
              </div>
              <p className="first-para">{contentData.description}</p>
              {contentData.quote_text && (
                <blockquote>
                  ...
                  <p>{contentData.quote_text}</p>
                  <cite>{contentData.quote_author}</cite>
                </blockquote>
              )}
              ...
              <div className="row mb-100">
                <div className="col-lg-12">
                  <div className="details-navigation">
                    {contentData.previous_blog && (
                      <div className="single-navigation">
                        <Link
                          className="arrow"
                          href={`/community/community-details?community_id=${contentData.previous_blog.blog_id}`}
                        >
                          <svg
                            width={9}
                            height={15}
                        ...
                        <div className="content">
                          ...
                          <h6>
                            <Link
                              href={`/community/community-details?community_id=${contentData.previous_blog.blog_id}`}
                            >
                              {contentData.previous_blog.title}
                            </Link>
                          </h6>
                        </div>
                      </div>
                    ...
                    {contentData.next_blog && (
                      <div className="single-navigation two text-end">
                        <div className="content">
                          <Link
                            href={`/community/community-details?community_id=${contentData.next_blog.blog_id}`}
                          >
                            Next Post
                          </Link>
                          <h6>
              ...
              <div className="row">
                <div className="comment-area" id="comment-area">
                  <div className="comment-title">
                    <h4>Comments ({contentData?.comments.length})</h4>
                    <div className="dash" />
                  </div>
            ...
            <div className="col-lg-4">
              <div className="sidebar-area">
                <div className="single-widget mb-30">
                  ...
                  {contentData.recent_blogs &&
                    contentData.recent_blogs
                      ?.filter((e) => e.blog_id !== contentData.blog_id)
                      ?.map((recentContent, index) => (
                        <div
                          key={recentContent.blog_id}
                          className="recent-post-widget mb-[20px]"
                        >
                          <div className="recent-post-img">
                            <Link
                              href={`/community/community-details?community_id=${recentContent.blog_id}`}
                            >
                              <img
                                src={recentContent.cover_image}
                          ...
                          <div className="recent-post-content">
                            <Link href="/community">
                              {formatDateShort(recentContent.created_at)}
                            </Link>
                            <h6>
                              <Link
                                href={`/community/community-details?community_id=${recentContent.blog_id}`}
                              >
                                {recentContent.title.length > 40
                                  ? recentContent.title.substring(0, 40) + "..."
                                  : recentContent.title}
                              </Link>
                            </h6>
                          </div>
