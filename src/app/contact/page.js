    ...
    const fetchContactData = async () => {
      ...
      try {
        const response = await axios.get(
          `${base_url}/user/contact/select_contact.php`
        );

        if (response.data.status === "success") {
          // Filter only visible items and group by type
          const visibleItems = response.data.message.filter(
            (item) => item.hidden === "0"
          );

          const phones = visibleItems.filter((item) => item.type === "Phone");
          const emails = visibleItems.filter((item) => item.type === "Email");
          const location = visibleItems.find(
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${base_url}/user/contact/send_message.php`,
        {
          name: formData.name,
          ...
          <div className="row g-lg-4 gy-5">
            <div className="col-lg-5">
              ...
              {contactData.phones.length > 0 && (
                <div className="single-contact mb-10">
                  <div className="title">
                    <h6>Phone</h6>
                  </div>
                  <div className="icon">
              ...
              {contactData.location && (
                <div className="single-contact mb-10">
                  ...
                  <div className="content">
                    <h6>
                      <a href="#">{contactData.location.value}</a>
                    </h6>
                  </div>
                </div>
              )}
