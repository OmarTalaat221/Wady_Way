...
const Header = ({ currentLocale }) => {
  ...
  const [, startTransition] = useTransition(); // For smooth async updates
  const pathname = usePathname();
  useEffect(() => {
    document.cookie = `lang=${lang}; path=/; max-age=31536000`; // Set client-side cookie
  }, [lang]);
  ...
  useEffect(() => {
    ...
    if (savedFavorites) {
      ...
      // Count true values in favorites object
  ...
  const changeLanguage = async (newLang) => {
    setLang(newLang); // Update state instantly
            ...
            >
              ...
              >
                {navData.map((data, index) => {
                  ...
                  return (
                    <li
                      key={id + "-" + index} // Use 'id' directly as key
                      style={{ color: "white" }}
                      className={`${
                      ...
                      {subMenu && (
                        ...
                        >
                          {subMenu.map((subItem) => (
                            ...
                            >
                              {subItem.subMenu ? (
                                // No navigation if subMenu exists
                                <div
                    ...
                    >
                      <div className="swiper-wrapper">
                        {destinaiton_sidebar_data.map((data, index) => {
                          const { id, img, tour_palce, tour_type, place } =
                            data;
                          return (
                            <SwiperSlide
